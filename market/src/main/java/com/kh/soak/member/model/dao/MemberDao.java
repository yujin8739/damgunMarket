...
//		return result;
	...
	public List<Member> selectHistoryMember(SqlSessionTemplate sqlSession, int userNo) {
	 ...
	 // Method to update user rank (increase or decrease)
    public int updateUserRank(SqlSessionTemplate sqlSession, Map<String, Object> params) {
        return sqlSession.update("memberMapper.updateUserRank", params);
    }

    // Method to get user rank
    public int selectUserRank(SqlSessionTemplate sqlSession, int userNo) {
        return sqlSession.selectOne("memberMapper.selectUserRank", userNo);
    }
