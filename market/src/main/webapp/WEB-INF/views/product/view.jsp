...
<body>
	...
	<p id="category">
		<strong>카테고리:</strong> ${product.bigCate} > ${product.midCate} >
		${product.smallCate}
	</p>
	<div class="product-detail">
		<p id="enrollStatus"
			style="cursor: pointer; color: green; font-size: 16px;"></p>
		<h1
			...
			<%-- 담구기 기능 추가 끝 --%>
			<span id="favoriteStar"
				style="cursor: pointer; color: gold; font-size: 24px;">☆</span>
		</h1>

		<p id="updateTime">
			<strong>업데이트:</strong> ${product.updateTime}
		</p>
		...
		<div id="json-data" data-json='${fileListJson}'
			style="display: inline-block;"></div>
		<div class="product-info">
			<p>
				<strong>가격:</strong> ${product.pdPrice} 원
			</p>
			<p>
				<strong>설명:</strong>
			</p>
			<p style="white-space: pre-wrap;">${product.pdBoard}</p>
			...
			<div id="stationSelectionWrapper"
			...
			<c:if
				test="${sessionScope.loginUser.userNo == product.userNo && enrollStatus != '판매완료'}">
				<div style="margin-top: 20px; text-align: right;">
					<div id="editBtnBox"
						style="display: flex; gap: 10px; text-align: right; justify-content: flex-end;">
						<form action="${pageContext.request.contextPath}/product/delete"
							method="post" onsubmit="return confirm('정말 삭제하시겠습니까?');"
							style="margin: 0;">
							<input type="hidden" name="pdNum" value="${product.pdNum}" /> <input
								type="hidden" name="userNo"
								value="${sessionScope.loginUser.userNo}" />
							<button id="deleteBtn" type="submit"
								style="background-color: #ff5c5c; color: white; border: none; padding: 10px 16px; border-radius: 6px;">
								삭제</button>
						</form>
						<form action="${pageContext.request.contextPath}/product/pdedit"
							method="get" onsubmit="return confirm('정말 수정하시겠습니까?');"
							style="margin: 0;">
							<input type="hidden" name="pdNum" value="${product.pdNum}" /> <input
								type="hidden" name="userNo" value="${product.userNo}" />
							<button id="editBtn" type="submit"
								style="background-color: green; color: white; border: none; padding: 10px 16px; border-radius: 6px;">
								수정</button>
						</form>
					</div>
			...
			<c:if test="${sessionScope.loginUser.userNo != product.userNo}">
				<div style="margin-top: 20px; text-align: right;">
					<input type="hidden" name="pdNum" value="${product.pdNum}" /> <input
						type="hidden" name="userNo" value="${product.userNo}" /> <input
						type="hidden" name="enrollNo"
						value="${sessionScope.loginUser.userNo}" /> <input type="hidden"
						name="userNo" value="${product.userNo}" /> <input type="hidden"
						name="enrollNo" value="${sessionScope.loginUser.userNo}" />
					<button id="chatButton" type="button"
						onclick="startChatWithSeller(${product.userNo}, ${sessionScope.loginUser.userNo})"
						style="cursor: pointer; background-color: black; color: white; border: none; padding: 10px 16px; border-radius: 6px;">
						채팅하기</button>
					<button id="tradeButton" type="button"
						onclick="doEnrollProcess(${product.pdNum}, ${product.userNo},${sessionScope.loginUser.userNo},'거래신청')"
						style="display: none; cursor: pointer; background-color: green; color: white; border: none; padding: 10px 16px; border-radius: 6px;">
						거래신청</button>
					<button id="enrollEndButton" type="button"
						onclick="doEnrollProcess(${product.pdNum}, ${product.userNo},${sessionScope.loginUser.userNo},'거래신청')"
						style="display: none; cursor: pointer; background-color: gray; color: white; border: none; padding: 10px 16px; border-radius: 6px;">
						신청완료</button>
				</div>
			</c:if>
		</div>
...
$('#dampgugiButton').on('click', function() {
    ...
    // 로그인 유저 번호를 안전하게 가져오는 방법 (삼항 연산자 없이)
    var currentUserNo;
    var loginUserExists = '${sessionScope.loginUser != null}'; // 'true' 또는 'false' 문자열로 렌더링
    if (loginUserExists === 'true') {
        currentUserNo = parseInt('${sessionScope.loginUser.userNo}');
    } else {
        currentUserNo = null; // 로그인하지 않은 경우 null로 설정
    }

    const productSellerNo = ${product.userNo};
    const pdNum = ${product.pdNum};

    if (currentUserNo === null) { // null인지 직접 확인
        alert('로그인 후 이용해주세요.');
        return;
    }
